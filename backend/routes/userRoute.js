    ...
    cloudinary.uploader.upload(picon, async (err, result) => {
      if (err) {
        ...
        if (results.error) {
          ...
          if (client) {
            res.status(500).json({
              warning: "this is registored for the for the client account ",
            });
          } else {
            if (user) {
    ...
    if (user) {
      if (user.otpexpire < Date.now()) {
        ...
        return res.status(200).json({
          success: true,
          msg: "your account verification is done ",
          data: user.email,
        });
      }
    } else {
...
router.post(`/setProfile/:${req.user.email}`, async (req, res) => {
  try {
...
router.post("/login", async (req, res) => {
  try {
    ...
    console.log(user);
    if (client) {
      return res.status(500).json({ warning: "client" });
    }
    if (!user) {
